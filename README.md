...
2. 每月 schedule 和手动 `workflow_dispatch` 都固定使用 GitHub-hosted Linux runner，通过 Xvfb 启动全新、有界面的真实 Chrome；独立的 `Local Self-Hosted Telecom Claim` 仅保留给本地诊断，不参与正式定时任务。
...
如果你走的是 PushPlus 直连 OpenAPI，建议同时配置：

- `PUSHPLUS_TITLE_KEYWORD`：先按 PushPlus 标题预筛，减少详情请求；
- `PUSHPLUS_KEYWORD`：再按标题 + 详情正文筛选，比如 `北京电信掌上营业厅` 或 `办理提醒`；
- `SMS_SENDER=10001`：从详情里的“发件号码/发件人”字段做二次校验。
...
这个脚本只输出消息 ID、sender、时间、是否命中登录/确认短信，不输出短信正文或验证码。
...
4. 先手动触发 GitHub-hosted 的 `Monthly Beijing Telecom Claim` workflow，设置 `probe_only=true`、`dry_run=true`；该模式只加载登录滑块，不提交滑块，也不发送短信。
